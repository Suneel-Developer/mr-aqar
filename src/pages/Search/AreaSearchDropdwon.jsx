...
const DropdownWithTags = () => {
    ...
    return (
        <div className="relative w-full mx-auto" ref={dropdownRef}>
            ...
            {/* Dropdown Body */}
            {isOpen && (
                <div className="fixed md:absolute top-0 md:top-full right-0 md:mt-2 w-full bg-white shadow-2 rounded-xl max-h-screen md:max-h-96 dropdown-scrollbar overflow-y-auto z-50 py-2 border border-[#dcdcdc] space-y-1">
                    <div className="fixed z-50 top-0 w-full p-6 pb-3 bg-white block md:hidden">
                        <div className='h-14 w-full flex items-center relative'>
                            <FaArrowRight onClick={() => setIsOpen(false)}  size={18} className='absolute top-0 bottom-0 right-4 m-auto z-30 text-[#3a7bb7]' />
                            <input type="text" className='relative block w-full h-full rounded-lg border pr-12' placeholder='أكتب المنطقة للبحث' />
                        </div>
                    </div>
                    ...
                    {filteredOptions.map((option) => (
                        <div
                            ...
                            className={`flex relative items-center justify-between py-2 px-2 mx-4 md:mx-2 rounded-lg cursor-pointer ${isSelected(option.id) ? "bg-[#e8f0f7]" : "bg-white"
                                }`}
                        >
