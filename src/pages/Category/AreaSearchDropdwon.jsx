...
const DropdownWithTags = () => {
    ...
    return (
        <div className="relative w-full mx-auto" ref={dropdownRef}>
            {/* Dropdown Header */}
            <div
                className={`relative w-full bg-white pe-11 cursor-text py-[7px] overflow-hidden transition-all ${
                    isOpen ? "border-blue-500 border" : "border-gray-300 border"
                ...
                <div className="absolute top-full mt-2 w-full bg-white shadow-lg rounded-xl max-h-96 dropdown-scrollbar overflow-y-auto z-10 py-2 border border-[#dcdcdc] space-y-1">
                    {filteredOptions.map((option) => (
                        <div
                            ...
                            className={`flex relative items-center justify-between py-2 px-2 mx-2 rounded-lg cursor-pointer ${
                                isSelected(option.id) ? "bg-[#e8f0f7]" : "bg-white"
                            }`}
                        >
